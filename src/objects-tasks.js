...
function sellTickets(queue) {
  ...
  const changeArr = queue.slice(0, queue.length - 1);
  const sum = changeArr.reduce((acc, cur) => acc + cur, 0);
  return sum >= end;
}
...
function Rectangle(width, height) {
  ...
  Rectangle.prototype.getArea = function calcArea() {
...
function sortCitiesArray(arr) {
  const res = arr.sort((a, b) => {
    return (
      a.country[0].charCodeAt() - b.country[0].charCodeAt() ||
      a.city[0].charCodeAt() - b.city[0].charCodeAt()
    );
  });
  return res;
}
